...
namespace DataLayer.Dashboard
{
    public class DashboardDbLayer
    {
        ...
        {   
           return _db.Pr_Sales_Order_List(_companyId)
               .Select(_data=>new ViewModel.Dashboard.SalerOrderinfo { Customer_Name = _data.Customer_Name, Contact_No = _data.Contact_No,
                   Order_No = _data.Order_No, Order_Date = _data.Order_Date.ToShortDateString(), Request_Delivery_Date = _data.Request_Delivery_Date.ToShortDateString(),
                   Status = _data.Status, Amount = _data.Amount });
        ...
        {
            return  _db.Pr_Purchase_Order_List(_companyId).Take(No_of_Records)
                .Select(_data => new ViewModel.Dashboard.Purchaseinfo { Supplier_Name = _data.Customer_Name, Contact_No = _data.Contact_No, 
                    Order_No = _data.Order_No, Order_Date = _data.Order_Date, Request_Delivery_Date = _data.Request_Delivery_Date, Status = _data.Status, Amount = _data.Amount });
        ...
        public IEnumerable<ManageStockInfo> ManageStock()
        {
            ...
            return _db.SP_ManageStock(_companyId).Take(10)
                .Select(m => new ManageStockInfo { Id = m.Id, Name = m.Name, Item_Code = m.Item_Code, Min_Qty = m.Min_Qty, Total_Qty = m.Total_Qty,
                    Selling_Rate = m.Selling_Rate, Purchase_Rate = m.Purchase_Rate, ManageStock = m.ManageStock, Status = m.Status });
